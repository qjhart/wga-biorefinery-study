...
create or replace function msw_summary_chart(t brfn) 
returns TEXT AS $$
DECLARE
total float;
BEGIN
RETURN 'http://chart.apis.google.com/chart?chxs=0,676767,13&chxt=x&chs=300x225&cht=p&chco=008000,3399CC,FFFF88'||
       '&chds=0,'||total||
       '&chd=t:'||
coalesce(t.msw_wood,0)||','||coalesce(t.msw_paper,0)||','||coalesce(t.msw_constr_demo,0)||','||coalesce(t.msw_yard,0)||','||coalesce(t.msw_food,0)||','||coalesce(t.msw_dirty,0)||
'&chdlp=l&chl=msw_wood|msw_paper|msw_constr_demo|msw_yard|msw_food|msw_dirty&chma=5,5,5,5&chtt=Feedstocks';
END;
$$ LANGUAGE plpgsql;
